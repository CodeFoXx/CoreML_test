...
//  NotificationTarget
//
//  Created by Осина П.М. on 14.03.18.
//
...
//            contentHandler(bestAttemptContent)
        ...
        guard let attachment = UNNotificationAttachment.create(imageFileIdentifier: "image.gif", data: imageData, options: nil) else {
            return failEarly() } content.attachments = [attachment] contentHandler(content.copy() as! UNNotificationContent)
        }
    ...
    //Actions
        ...
        let category = UNNotificationCategory(identifier: "NEWS_CATEGORY", actions: [likeAction, addToCartAction], intentIdentifiers: [], options: [])
